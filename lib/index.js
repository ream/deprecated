...
function renderTemplate(template, context) {
  ...
  start = start
    ...
    `<script>window.__REAM__=${serialize({ state: context.state, data: context.data }, { isJSON: true })}</script>` +
...
class Ream extends EventEmitter {
  ...
  prepare() {
    ...
    if (this.dev) {
      ...
      return pipe
        ...
        .then(port => {
          ...
          return this.setWebpackConfig().then(() => ({ port, host }))
        })
    ...
    const bundle = require(this.getCwd(
      this.options.outputFolder,
      'dist/vue-ssr-server-bundle.json'
    ))
    const clientManifest = require(this.getCwd(
      this.options.outputFolder,
      'dist/vue-ssr-client-manifest.json'
    ))
    this.template = fs.readFileSync(
      this.getCwd(this.options.outputFolder, 'dist/index.html'),
      'utf-8'
    )
  ...
  build(options) {
    return this.setWebpackConfig(options).then(() => {
  ...
  generate({ routes = [], homepage = '/' } = {}) {
      ...
      if (isType(routes, 'Object')) {
        return Object.keys(routes)
          .map(route => {
            ...
            const patterns = Array.isArray(routes[route])
              ? routes[route]
              : [routes[route]]
        ...
        parseRoutes(routes)
          ...
          .map(url => {
            ...
            return promisify(this.renderer.renderToString)(context)
              ...
              .then(html => {
                const file = this.getCwd(
                  this.options.outputFolder,
                  'dist' + handleUrl(url)
                )
                return fs.ensureDir(path.dirname(file)).then(() => {
    ...
    return this.build({ homepage })
      ...
      .then(() => {
        rm.sync(
          this.getCwd(this.options.outputFolder, 'dist', 'vue-ssr-bundle.json')
        )
  ...
  getRequestHandler() {
    ...
    routes['/_ream/*'] = (req, res) => {
      ...
      serveStatic(this.getCwd(this.options.outputFolder, 'dist'), !this.dev)(
        req,
        res,
    ...
    routes['/public/*'] = (req, res) => {
      ...
      serveStatic(this.getCwd('public'), !this.dev)(
        req,
        res,
        finalhandler(req, res)
      )
    ...
    routes['*'] = (req, res) => {
      ...
      const context = { url: req.url, req, data: {}, dev: this.dev }
