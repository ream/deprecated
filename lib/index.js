...
function renderTemplate(template, context) {
  ...
  end = `<script>window.__UNVUE__=${serialize({
    state: context.state,
    asyncData: context.serverCache && context.serverCache.get(`asyncData:${context.url}`),
    preFetchCache: context.serverCache && context.serverCache.get(`preFetchCache:${context.url}`)
...
class UNVUE extends EventEmitter {
  constructor(options = {}) {
    ...
    // Cache preFetchCache and asyncData
    this.serverCache = require('lru-cache')(Object.assign({
      max: 1000,
  ...
  setWebpackConfig(options) {
    ...
    const serverConfig = createConfig(Object.assign({}, this.options, {
      type: 'server',
      dev: this.dev
    }))
  ...
  getRequestHandler() {
    ...
    const routes = {
      ...
      '*': (req, res) => {
        ...
        const context = { url: req.url, serverCache: this.serverCache }
