...
function renderTemplate(template, context) {
  ...
  if (context.cache) {
    context.preFetchCacheKeys.forEach(key => {
      preFetchCache[key] = context.cache.get(key)
    })
    context.asyncDataKeys.forEach(key => {
      asyncData[key] = context.cache.get(key)
    })
  }

  end = `<script>window.__UNVUE__=${serialize(Object.assign({
    state: context.state
  }, preFetchCache, asyncData), { isJSON: true })}</script>` + end
...
class UNVUE extends EventEmitter {
  constructor(options = {}) {
    ...
    this.cache = require('lru-cache')(Object.assign({
      max: 1000,
