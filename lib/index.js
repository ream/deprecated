...
function createRenderer(bundle, options) {
  return require('vue-server-renderer').createBundleRenderer(bundle, Object.assign({
    runInNewContext: options.runInNewContext,
...
class Ream extends EventEmitter {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      runInNewContext: typeof options.runInNewContext === 'undefined' ? true : options.runInNewContext
    }
    ...
    return payload => {
      ...
      if (payload.bundle) {
        this.renderer = createRenderer(payload.bundle, {
          ...
          runInNewContext: this.options.runInNewContext
        })
      }
      if (payload.template) {
