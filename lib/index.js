...
class Ream extends EventEmitter {
  ...
  prepare() {
    ...
    const bundle = require(this.getCwd(this.options.outputFolder, 'dist/vue-ssr-bundle.json'))
  ...
  generate({
    ...
    const g = () => Promise.all(parseRoutes(routes)
      ...
      .map(url => {
        ...
        .then(html => {
          const file = this.getCwd(this.options.outputFolder, 'dist' + handleUrl(url) + '.html')
